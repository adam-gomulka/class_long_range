...
int input_read_parameters(struct file_content * pfc,
  ...
  class_call(input_read_parameters_species(pfc,ppr,pba,pth,ppt,
    ...
    if (pba->Omega0_scf != 0.) {
      ...
      //double H = pba->H0;
      double req_rho_tot = pow(pba->H0, 2);
      double h = pba->h;
      int iter = 0;
      ...
      // Start with some initial guess (e.g., 0.7)
      ...
      while (iter < MAX_OMEGA_LAMBDA_ITERS) {
        ...
        double omega0_chi = ba_temp.Omega0_chi * (m_chi(pba, phi_today) / m_chi(pba, ba_temp.phi_ini_scf))*h*h; // this is the present day DM density according to Eq. (3.6)
        ...
        double omega_tot = 0.0;
        omega_tot = omega0_b + omega0_g + omega0_ur + omega0_ncdm + omega0_chi + omega0_cdm + omega0_scf_temp;
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /* Commented out for now as we don't care for fluid nor curvature in our implementation */
  ...
  if (pba->Omega0_scf != 0.){
    ...
    if (flag1 == _TRUE_){
      if (string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
        ...
        if (pba->beta < 0) {
          class_stop(errmsg, "You have set beta < 0, which is not allowed for the scalar field initial conditions. Please set beta > 0.");
        }
        else if (pba->beta == 0) { /* just ordinary scalar field stuff here, in this case you pass initial condisitons as the last two entries of scf_parameters */
        ...
        /* LONG RANGE START */
          ...
          pba->phi_prime_ini_scf = pba->scf_parameters[pba->scf_parameters_size-1]*(-3/4)*pba->beta*(dlogm_chi(pba, pba->phi_ini_scf))*pba->Omega0_chi/sqrt(pba->Omega0_g+pba->Omega0_ur)*pba->H0;//pba->scf_parameters[pba->scf_parameters_size-1];
        }
        /* LONG RANGE END   */
      }
    }
  ...
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "You can only enter one of 'Omega_chi' or 'omega_chi'.");
  /* Complete set of parameters */
  if (flag1 == _TRUE_){
  ...
  class_test(pba->Omega0_chi<0,errmsg, "You cannot set the dark matter density to negative values.");
