...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case omega_dcdmdr:
      ...
      break;
    // case alpha_lrm_1:
    //   /* a guess taken from 2204.08484 and analytic formula (3.12)*/
    //   Omega_tot = ba.Omega0_g;
    //   // printf("Omega0_g = %e\n",ba.Omega0_g);
    //   Omega_tot += ba.Omega0_b;
    //   Omega_tot += ba.Omega0_ur;
    //   Omega_tot += ba.Omega0_cdm;
    //   Omega_tot += ba.Omega0_idm;
    //   Omega_tot += ba.Omega0_dcdmdr;
    //   Omega_tot += ba.Omega0_idr;
    //   Omega_tot += ba.Omega0_ncdm_tot;
    //   // printf("alpha_lrm_1 = %e\n", 16/(3*_PI_)*pow(pt.alpha_lrm_1, 2));
    //   // printf("Omega_cdm/Omega_m = %e\n", pow(ba.Omega0_cdm/ba.Omega0_m, 1));
    //   // printf("rho_scf/rho_tot = %e\n", 16/(3*_PI_)*pow(pt.alpha_lrm_1, 2)*pow(ba.Omega0_cdm/ba.Omega0_m, 2));
    //   Omega_tot += 16/(3*_PI_)*pow(pt.alpha_lrm_1, 2)*0.7;//pow(ba.Omega0_cdm/ba.Omega0_m, 2);
    //   // printf("Omega_tot = %e\n",Omega_tot);
    //   xguess[index_guess] = 1-Omega_tot-ba.Omega0_k;
    //   dxdy[index_guess] = -0.5;//32/(3*_PI_)*pt.alpha_lrm_1*0.7;//*ba.h*ba.h;
    //   printf("Initial guess: Omega0_lambda = %e, dxdy = %e\n", xguess[index_guess], dxdy[index_guess]);
    //   break;
    ...
    //   if (ba.scf_tuning_index == 1){
    //     xguess[index_guess] = sqrt(3.0/ba.Omega0_scf);
    //     //xguess[index_guess] = sqrt((6.0*ba.Omega0_scf)/pow(9*(Omega_r),0.75)/pow(ba.scf_parameters[1]*_eV_over_Mpc_*ba.H0, 0.5)); //ba.scf_parameters[1] is the ratio m/H0
    //     dxdy[index_guess] = -0.5*sqrt(3.0)*pow(ba.Omega0_scf,-1.5);
    //     //dxdy[index_guess] =  10*xguess[index_guess];
    //   }
    //   else{
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case omega_dcdmdr:
      ...
      break;
    // case alpha_lrm_1:
    //   double rho_scf, rho_tot, Omega_cdm, Omega_m, alpha_lrm_1_predicted;

    //   rho_scf = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_scf];

    //   rho_tot = 0.;
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_g];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_b];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_ur];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_cdm];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_idm];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_idr];
    //   rho_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_scf];

    //   Omega_cdm = ba.Omega0_cdm;
    //   Omega_m = ba.Omega0_m;

    //   alpha_lrm_1_predicted = sqrt( (3. * _PI_ / 16.) * (rho_scf / rho_tot) ) * (Omega_m / Omega_cdm);
    //   // Difference between predicted and target value:
    //   output[i] = alpha_lrm_1_predicted - pfzw->target_value[i];
    //   printf("Computed alpha_lrm_1 = %e, target = %e, diff = %e\n",
    //     alpha_lrm_1_predicted, pfzw->target_value[i], output[i]);
    //   // double Omega_tot=0.;
    //   // Omega_tot = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_g]/(ba.H0*ba.H0);
    //   // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_b]/(ba.H0*ba.H0);
    //   // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_ur]/(ba.H0*ba.H0);
    //   // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_cdm]/(ba.H0*ba.H0);
    //   // // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_idm]/(ba.H0*ba.H0);
    //   // // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_dcdm]/(ba.H0*ba.H0);
    //   // // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_idr]/(ba.H0*ba.H0);
    //   // Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_scf]/(ba.H0*ba.H0);
    //   // printf("Omega0_scf = %e\n",ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_scf]/(ba.H0*ba.H0));
    //   // printf("Omega0_cdm = %e\n",ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_cdm]/(ba.H0*ba.H0));
    //   // printf("Omega_tot = %e\n", Omega_tot);
    //   // //Omega_tot += ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_ncdm]/(ba.H0*ba.H0);
    //   // printf("Omega0_lambda_target = %e\n", pfzw->target_value[i]);
    //   // output[i] = (1-Omega_tot)-pfzw->target_value[i];
    //   // printf("Omega0_lambda - targeted_value = %e\n", output[i]);
    //   // ba.Omega0_lambda = 1-Omega_tot-ba.Omega0_k;
    //   break;
    // case Omega_scf:
...
int input_read_parameters(struct file_content * pfc,
  ...
  class_call(input_read_parameters_species(pfc,ppr,pba,pth,ppt,
    ...
    /** Automatically infer Omega0_lambda from all other densities, if not provided **/

    double dummy;
    int flag1;
    class_call(parser_read_double(pfc, "Omega0_lambda", &dummy, &flag1, errmsg),
               errmsg, errmsg);
  
    // if (flag1 == _FALSE_) {
    //   // User did not provide Omega0_lambda → we infer it after computing Omega0_scf
  
    //   struct precision pr_temp;
    //   struct background ba_temp;
  
    //   // Copy current background input
    //   ba_temp = *pba;
    ...
    //   // Initialize background with temporary structure to compute Omega0_scf
    //   class_call(background_init(&pr_temp, &ba_temp),
    //              ba_temp.error_message, errmsg);
  
    //   double Omega_tot = 0.;
  
    //   Omega_tot += ba_temp.Omega0_b;
    //   Omega_tot += ba_temp.Omega0_cdm;
    //   Omega_tot += ba_temp.Omega0_idm;
    //   Omega_tot += ba_temp.Omega0_dcdmdr;
    //   Omega_tot += ba_temp.Omega0_idr;
    //   Omega_tot += ba_temp.Omega0_ncdm_tot;
    //   Omega_tot += ba_temp.Omega0_ur;
    //   Omega_tot += ba_temp.Omega0_g;
    //   Omega_tot += ba_temp.Omega0_scf;
  
    //   pba->Omega0_lambda = 1.0 - Omega_tot - ba_temp.Omega0_k;
  
    //   printf("Inferred Omega0_lambda = %e (including Omega0_scf = %e)\n",
    //          pba->Omega0_lambda, ba_temp.Omega0_scf);
  
    //   if (pba->Omega0_lambda < 0.0) {
    //     class_stop(errmsg, "Inferred Omega0_lambda = %e < 0. Check total densities.", pba->Omega0_lambda);
    //   }
  
    //   // Cleanup
    //   background_free(&ba_temp);
    // }
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  class_read_double("alpha_lrm_1", ppt->alpha_lrm_1);
  class_read_double("alpha_lrm_2", ppt->alpha_lrm_2);
  ...
  //   // Force scalar field to use attractor initial conditions, if needed
  //   // if (ba_temp.scf_tuning_index != 1) {
  //   //   ba_temp.scf_tuning_index = 1;
  //   // }
